...
def SalidaControlador(alturas, eleccion, tipoManual, frec, amp, offset, fase, manualFijo,
    ...
    # Modo automático
        ...
        # Constantes
        fc = 1000             # cuando ya ya se defina fc en otras partes, esta linea se comenta
        pid1.set_PID_param(float(Kp), float(Ki), float(Kd), float(fc), float(Kw))
        pid2.set_PID_param(float(Kp), float(Ki), float(Kd), float(fc), float(Kw))
