    ...
    html.Section(className="section", style={'backgroundColor': 'black'}, children=[
        ...
        html.Div(id='Modos', className="container",style={'color': colors['text'], 'backgroundColor': 'black'} , children=[
            ...
            html.Div(id='Automatico', className="container", style={'color': colors['text'], 'background-color': 'black', 'border': '2px solid gray'}, children=[
                ...
                html.Div(id='constantes1', className="columns is-centered", children=[
                    html.Div(id='P', className="column has-text-centered", children=[
                        html.Label('Proportional'),
                        dcc.Input(id='Kp', placeholder='Enter a value', type='text', value='50')
                    ]),
                    html.Div(id='I', className="column has-text-centered", children=[
                        html.Label('Integral'),
                        dcc.Input(id='Ki', placeholder='Enter a value', type='text', value='0')
                    ])
                ]),
                html.Div(id='constantes2', className="columns is-centered", children=[
                    html.Div(id='D', className="column has-text-centered", children=[
                        html.Label('Derivative'),
                        dcc.Input(id='Kd', placeholder='Enter a value', type='text', value='0')
                    ]),
                    html.Div(id='W', className="column has-text-centered", children=[
                        html.Label('Anti wind-up'),
                        dcc.Input(id='Kw', placeholder='Enter a value', type='text', value='0')
                    ])
                ])
            ])
        ]),
...
def toggle_mode(mode):
    style_manual = {'backgroundColor': 'black', 'padding': '20px', 'border': '2px solid gold', 'borderRadius': '10px', 'color': colors['text']}
    if mode == 'Manual': return {'display': 'block'} | style_manual, {'display': 'none'} | style_manual
        # lo mismo que la linea anterior pero estableciendo el color de fondo como negro

    elif mode == 'Automatico':
        return {'display': 'none'} | style_manual, {'display': 'block'} | style_manual
...
def Alarma(n):
    global eventoColor
    if eventoColor != 0:
        style = {'backgroundColor': 'black', 'width': '80%', 'height': '70px', 'paddingTop': '25px', 'margin': 'auto',
...
def TextoAlarma(n):
    ...
    if eventoTexto != 0:
        ...
        res = 'Alrama Activa: {}: {}'.format(mensaje[1], round(float(mensaje[2]), 2))
    else:
        res = 'Alarma Inactiva'
    eventoTexto = 0
    return res
